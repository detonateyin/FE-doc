# JavaScript执行上下文-执行栈（转载）
>原文：[《JavaScript执行上下文-执行栈》](https://juejin.cn/post/6844904199063339015)  
作者：TianTianUp  
链接：https://juejin.cn/post/6844904199063339015  
来源：掘金
...
在 ES6 中，**词法** 环境和 **变量** 环境的区别在于前者用于存储 函数声明和变量（ `let` 和 `const` ） 绑定，而后者仅用于 存储变量（ `var` ） 绑定。
